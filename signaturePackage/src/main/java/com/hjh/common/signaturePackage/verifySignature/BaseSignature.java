...
public abstract class BaseSignature {
	 ...
	 * @return boolean
	 */
	public static boolean verifyWithPemStream(byte[] data, byte[] signedData, InputStream pem)
			throws Exception {
		byte[] bytes = StreamToByteUtil.getByte(pem);
		Signature instance = SignatureUtil.getSignature();
		PublicKey publicKey = getPublicKey(bytes);
		instance.initVerify(publicKey);
		instance.update(data);
		return instance.verify(signedData);
	}

	static PublicKey getPublicKey(byte[] bytes) {
